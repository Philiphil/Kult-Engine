...
abstract class shop{
	...
	static public function get_current_shop()
	{
		$d = new daoGenerator(new magasin());
		return $d->select($_SESSION["cart_location"]);
	}
	static public function reset_shop()
	{
	...
	static public function create_shop()
	{
	...
	static public function admin()
	{
		...
		$o = [];
		$o["admin"] = $header;
		if(count($c)>0) $o["commandes"]=count($c);
	...
	$html.= "<p>".$key->_nom[text::get_lang()]." <a href='".constant("url")."/inventory/".$key->_id."'>".shopText::get_text("inventaire")."</a> <a href='".constant("url")."/del_magasin/".$key->_id."del_magasin'>".shopText::get_text("supprimer")."</a></p>";
		...
		foreach ($us as $u) {;
			...
			$ls .= "<br>".$cl['_ville'];
			$ls .= "<br>".$cl['_pays']."</p>";
			if ($u->_statut == 0) $ls .= "<a href='".constant("url")."/set_commande/$u->_id/1'>".shopText::get_text("subcommande")."</a>";
			if($u->_statut ==1){
				$s = new secureSerial();
				$e = $s->serialize($u);
				$e = base64_encode($e);
