...
class soros_bot{
	...
	public static function require(){
		...
		if(self::$db_records) self::$dao_records = new daoGenerator(new \stock_record()); 
	...
	public static function actualise_bag($a=false,$o=false){
		...
		foreach($bal as $val => $key){
			if(in_array($val, self::$no_follow)) continue;
			if(self::$test != null && $val != self::$test) continue;
			...
			if($key["available"] == 0 && !$e) continue;
			$prix = !!self::$ticker[$val.self::$etalon] ? self::$ticker[$val.self::$etalon] : self::$ticker[$val."BNB"] ;
			if(!$e){
				$e = new \stock();
				$e->_sym = $val;
				$e->_prix_achat = $prix;
			...
			if($e->_max_qtt == 0) $e->_max_qtt = $key["available"];
			if($e->_round == 0 && $val != self::$etalon){
				$c = !!self::$ticker[$val.self::$etalon] ? self::$etalon : "BNB" ;
				foreach(self::$depth['symbols'] as $depth)
				{
					if($depth['symbol'] != $val.$c)continue;
	...
	public static function analyse_crypto($crypto){
		$ecart = "   ";
		if(self::$test != null && $crypto->_sym != self::$test) return false;
		if(in_array($crypto->_sym, self::$no_follow)) return false;

		self::$ticker = self::$api->prices();

		$prix = !!self::$ticker[$crypto->_sym.self::$etalon] ? self::$ticker[$crypto->_sym.self::$etalon] : self::$ticker[$crypto->_sym."BNB"] ;

		$c = !!self::$ticker[$crypto->_sym.self::$etalon] ? self::$etalon : "BNB" ;
		
		if($prix > $crypto->_max_price) $crypto->_max_price = $prix;

		if($prix < $crypto->_min_price) $crypto->_min_price = $prix;
	...
	public static function loop(){
		...
		foreach (self::$cryptos as $val) {
			...
			if( $val->_qtt > $val->_min_qtt && 
				...
				if(self::$visuel) echo  "s"."    ".$diff2 ." ". self::$min_sell_var."   ".self::$min_sell_val." ".$diff3."\n";
				...
				if(	
					...
					if($qtn != 0){
						...
						if(!isset($transaction["code"]) && !isset($transaction["msg"])) $val->_prix_vente = $result[0];
					}
				}
			}else if($result[0] > $val->_min_price	&&
					$result[0] < $val->_prix_vente &&
					$val->_qtt < $val->_max_qtt	
			){
				$diff3 = \stock::compare($val->_prix_vente ,$result[0]);
				if(self::$visuel) echo "b"."    ". $diff2 ."    ".$diff3."\n";
				...
				if(!isset($transaction["code"]) && !isset($transaction["msg"]))$val->_prix_achat = $result[0];
				}
			}
