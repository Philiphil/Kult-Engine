	...
	public function loop()
	{
		...
		while (true) {
			foreach ($this->_events as $key) {
				if( get_class($key) == "netEvent") $key->listen();
				if( get_class($key) == "event" && call_user_func($key->_listenner) )call_user_func_array($key->_callback, $key->_args);
			}
		}
	}
}
...
class netEvent extends event{
	...
	public function listen(){
		if($this->_socket === null) $this->prepare_socket();		
